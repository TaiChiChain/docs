...
以下步骤都将以Gemini测试网为例，如果您想在主网或其他测试网上尝试，可在[资源](/zh/documentation/getting-started/resources/resources)页查看其他链的相关资源。
...
以 Gemini 测试网为例, Hardhat config 配置如下所示:
...
调用如下命令部署合约到对应的网络上，这里我们将合约部署到 AxiomLedger 的 Gemini 测试网上作为示范。
...
由于目前 Uniswap 仅支持以太坊主网以及其测试网，因此您需要修改 `src/constants.ts` 中的 `ChainId[TESTNET]` 为你需要的`链ID`。此处以AxiomLedger 测试网`Gemini`为例，可将`链ID`修改为 `23413`：
...
如下以 `src/tokens/testnet.json` 为例，请将上文部署 token 合约之后生产的 `WETH_ADDR`、`AXMTOKEN1_ADDR` 等填入相应的位置，测试令牌的 `logoURI` 可以自定义，以`Gemini`测试网为例，您可以修改 `chainId` 为 `23413`。
...
修改环境配置。这里以 AxiomLedger 的 Gemini 测试网络为例，将项目目录下的 `.env` 和 `.env.production` 文件中的相应的条目修改为如下值：
...
现在您可以体验构建在 AxiomLedger 的 Gemini 测试网上的 Uniswap@V2 了，尝试一下添加流动性吧！
