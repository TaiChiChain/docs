...
The following steps will take the Gemini testnet as an example. If you want to try it on the mainnet or other testnets, you can view related resources on the [Resources](/en/documentation/getting-started/resources/resources) page.
...
The account private key is used for deploying contracts on the chain. It can be safely used by setting it as an environment variable. Use the command `export YOUR_PRIVATE_KEY = 'YOUR_PRIVATE_KEY'` to write it to the environment variable. Use the `Gemini` testnet as an example, the `Hardhat config` is as follows:

```typescript
const config: HardhatUserConfig = {
  defaultNetwork: "gemini",
  networks: {
    gemini: {
...
Run the following command to deploy the contracts to the corresponding network. Here, we'll deploy the contracts to the `Gemini` testnet as an example.
...
```shell
npx hardhat run scripts/deployUniswapV2.js --network gemini
npx hardhat run scripts/deployUniswapTokens.js --network gemini
```
...
Since currently Uniswap only supports the Ethereum mainnet and its testnets you need to modify the `ChainId[TESTNET]` in `src/constants.ts` to your desired `chain ID`. Here, we'll take the `AxiomLedger` testnet `Gemini` as an example, and you can change the chain ID to `23413`.
...
Adjust the environment configuration. Taking `AxiomLedger` `Gemini` testnet as an example, modify the corresponding entries in the `.env` and `.env.production`files to the following values:

```text
REACT_APP_CHAIN_ID="23413"
REACT_APP_NETWORK_URL="https://rpc4.gemini.axiomesh.io"
```
...
Now you can experience Uniswap@V2 built on `AxiomLedger` `Gemini` testnet.
